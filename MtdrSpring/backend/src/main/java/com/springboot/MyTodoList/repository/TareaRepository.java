...
public interface TareaRepository extends JpaRepository<Tarea, Long> {
    ...
    // Punto 5: Mostrar la tabla de tasks del SPRINT actual haciendo JOIN con usuario
    @Query(value = "SELECT t.ID_TAREA as idTarea, t.NOMBRE as nombreTarea, " +
                   ...
                   "LEFT JOIN ESTADOS_TAREA e ON t.ID_ESTADO = e.ID_ESTADO " +
                   "JOIN SPRINTS s ON t.ID_SPRINT = s.ID_SPRINT " +
                   "WHERE t.ID_SPRINT = (SELECT ID_SPRINT FROM SPRINTS ORDER BY FECHA_INICIO DESC FETCH FIRST 1 ROWS ONLY)",
           nativeQuery = true)
